    ...
    <div>
      <div class="grid grid-cols-3 gap-5 p-10 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-3">
        <div class="w-full lg:max-w-full lg:flex" v-for="episode in episodes" :key="episode.id">
          <div class="flex-none h-48 overflow-hidden text-center bg-cover rounded-t lg:h-auto lg:w-48 lg:rounded-t-none lg:rounded-l" v-bind:style="{ 'background-image': 'url(' + episode.chapter.logo_path + ')' }" alt="Logo"></div>
